  ...
  <div class="flex divide-x text-sm z-10">
            ...
            <li v-for="nav in navigations" class="px-4" :key="nav.name">
              <div v-if="nav.children" class="space-y-1">
                ...
                >
                  ...
                  <MaterialIcon
                    ...
                    v-bind:class="
                      nav.open || (nav.open && nav.href.includes($route.href)) ? 'rotate-90' : ''
                    "
                    class="w-3 h-3 rounded-sm"
                  />
                </div>
                <ul
                  v-show="nav.open || (nav.open && nav.href.includes($route.href))"
                  class="space-y-1"
                >
                  <li v-for="sub_nav in nav.children" :key="sub_nav.name">
                    <RouterLink :to="sub_nav.href">
                      ...
                      >
                        <MaterialIcon :icon="sub_nav.icon" class="w-5 h-5" />
              ...
              <div v-else>
                <RouterLink :to="nav.href">
                  <div
                    v-bind:class="[
                      $route.path == nav.href ? 'bg-accent text-primary' : 'hover:bg-zinc-100',
                      'flex items-center gap-2 rounded-md cursor-pointer px-3 py-1.5'
                    ]"
                  >
                    <MaterialIcon :icon="nav.icon" class="w-5 h-5" />
    ...
    <div v-show="sidebar == false" class="w-16 divide-y flex flex-col">
      <div class="min-w-full h-12 flex items-center justify-center relative">
        <div class="flex items-center justify-center w-8 h-8 bg-secondary border rounded-full">
          ...
          <ul class="space-y-1 my-2">
            <li v-for="nav in navigations" class="group relative px-2" :key="nav.name">
              <div v-if="nav.children">
                ...
                >
                  <div class="flex justify-between items-center gap-x-3 rounded-r-md px-4 py-2">
                    <div class="flex items-center gap-2">
                      <span>{{ nav.name }}</span>
                    </div>
                    <MaterialIcon icon="ArrowForwardIosRound" class="w-3 h-3 rounded-sm" />
                  </div>

                  <ul class="space-y-1">
                    <li v-for="sub_nav in nav.children" :key="sub_nav.name">
                      <RouterLink :to="sub_nav.href">
                        <div class="flex items-center gap-2 rounded-md cursor-pointer px-4 py-2">
                          <span>{{ sub_nav.name }}</span>
                        </div>
                      </RouterLink>
                    </li>
              ...
              <div v-else>
                ...
                >
                  <label class="flex justify-between items-center gap-x-3 rounded-r-md px-4 py-2">
                    <span>{{ nav.name }}</span>
                  </label>
                </div>
              </div>
